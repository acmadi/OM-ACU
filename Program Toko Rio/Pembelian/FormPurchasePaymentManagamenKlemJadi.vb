...
Public Class FormPurchasePaymentManagamenKlemJadi
    ...
    Private Sub setData()
        Try
            ...
            sql = " select KdPurchasePayment,DATE_FORMAT(TanggalPurchasePayment, '%m/%d/%Y') Tanggal, " & _
            ...
            Dim readerPayment = execute_reader(sql)
            ...
            Dim reader = execute_reader(" Select MB.KdBarang, NamaBarang, Ukuran, " & _
                                        " Harga, Qty, Qty_real, Disc, StatusBarang, Satuan " & _
                                        " from TrPurchasePaymentDetail payment " & _
                                        " Join MsBarang MB On payment.KdBahanMentah = MB.KdBarang " & _
                                        " where kdPurchasePayment = '" & PK & "' " & _
            ...
            Do While reader.Read
                ...
                gridBahanMentah.Rows.Item(gridBahanMentah.RowCount - 1).Cells("clmNamaBarang").Value = reader("NamaBarang")
    ...
    Private Sub FormMsSupplier_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If jenis_payment = "klem_jadi" Then
            Me.Text = "Pembayaran Klem Jadi"
        Else
            Me.Close()
        End If
        clear_variable_array()
        gridBahanMentah.Columns("clmQty").HeaderText = "Qty ( Pack )"
    ...
    Function SavePaymentDetail(ByVal flag As String)
        ...
        For i As Integer = 0 To gridBahanMentah.RowCount - 1
            Dim statusDetail = 0
            Dim harga = Val(gridBahanMentah.Rows.Item(i).Cells("clmHarga").Value.ToString.Replace(".", "").Replace(",", ""))
            ...
            Try
                If PK = "" Then
                    sql = " insert into  trPurchasePayment ( KdPurchasePayment, " & _
                          ...
                          " '" & Trim(kdSupplier) & "'," & _
                          " '" & Val(Grandtotal) & "','" & flag & "', " & _
                          " '" & txtNote.Text & "','" & kdKaryawan & "','" & cmbBayar.Text & "', " & _
                          " '" & jenis_payment & "', '" & Val(Jumlah) & "', '" & txt_disc_payment.Text & "' ) "
                    execute_update_manual(sql)
                Else
                    sql = " update trPurchasePayment set " & _
                          " TanggalPurchasePayment = '" & dtpPayment.Value.ToString("yyyy/MM/dd HH:mm:ss") & "'," & _
                          " No_PB = '" & cmbPB.Text & "'," & _
                          " kdSupplier = '" & Trim(kdSupplier) & "'," & _
                          " TotalPurchasePayment = '" & Val(Grandtotal) & "', " & _
                ...
                execute_update_manual(" delete from TrPurchasePaymentdetail " & _
                                      " where  kdPurchasePayment = '" & txtID.Text & "'")
    ...
    Private Sub cmbpb_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmbPB.SelectedIndexChanged
        If cmbPB.SelectedIndex <> 0 Then
            Dim reader = execute_reader(" SELECT MB.KdBarang, NamaBarang, Ukuran, " & _
                                        ...
                                        " JOIN MsBarang MB ON pb.KdBahanMentah = MB.KdBarang " & _
                                        " JOIN MsSupplier MS ON MS.KdSupplier = trheaderpb.KdSupplier " & _
                                        " WHERE trheaderpb.No_PB = '" & cmbPB.Text & "' " & _
                                        " AND StatusPaid <> 1 " & _
                                        " GROUP BY MB.KdBarang, trheaderpb.No_PB,Harga " & _
                                        " HAVING(Qty <> 0) " & _
                                        " ORDER BY NamaBarang asc ")
